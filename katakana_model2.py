...
class KatakanaCNN2:
    ...
    def __init__(self):
        ...
        self.params = {}
        # 第1層: Conv1
        self.params['W1'] = np.random.randn(filter_num1, 1, filter_size, filter_size) * np.sqrt(2.0 / (1 * filter_size * filter_size))
        self.params['b1'] = np.zeros(filter_num1)
        
        # 第2層: Conv2
        self.params['W2'] = np.random.randn(filter_num2, filter_num1, filter_size, filter_size) * np.sqrt(2.0 / (filter_num1 * filter_size * filter_size))
        self.params['b2'] = np.zeros(filter_num2)
        
        # 第3層: Conv3
        self.params['W3'] = np.random.randn(filter_num3, filter_num2, filter_size, filter_size) * np.sqrt(2.0 / (filter_num2 * filter_size * filter_size))
        self.params['b3'] = np.zeros(filter_num3)
        
        # Conv->Pool後のサイズを計算
        # 入力: 28x28
        # Conv1(3x3, pad=1) -> 28x28 -> Pool(2x2) -> 14x14
        ...
        # 第1畳み込み層ブロック
        self.layers.append(Convolution(self.params['W1'], self.params['b1'], stride=1, pad=1))
